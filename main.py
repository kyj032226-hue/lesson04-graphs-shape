...
st.subheader("💡 이 그래프로 알 수 있는 것")
st.write(
    "한국, 미국 등 주요 제작 국가별로 어떤 장르의 영화가 주로 제작·개봉되었는지 영화 편수 규모를 계층 구조로 한눈에 비교할 수 있습니다."
)

st.divider()
```eof

제작 국가(`nation`)에서 장르(`main_genre`)로 이어지는 계층형 선버스트 차트가 성공적으로 추가되었습니다! 각 조각의 크기는 해당 국가 및 장르의 영화 편수를 나타냅니다.
